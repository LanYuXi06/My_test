...
void Chassis_Init(void)//初始化四个舵轮，全部置零
{
...
void Chassis_TestControlFromRemote(void)//将四个舵轮电流数据打包通过FDCAN1发送0X6FF 16字节给G0
{
    ...
    if (RC.s1 == SWERVE_TEST_ENABLE_SW)
    {
			...
			for(uint8_t i=0;i<SWERVE_MODULE_NUM;i++)
			{
        wheel_current = (int32_t)(vx-wz*y[i]);//平动速度
				wheel_vx=wheel_current;
        yaw_current = (int32_t)(vy+wz*x[i]);//自传速度
				wheel_vy=yaw_current;
				wheel_target[i] = -sqrtf(wheel_vx * wheel_vx + wheel_vy * wheel_vy);
        angle_target[i] = atan2f(wheel_vy, wheel_vx) * 57.29578f;//转成角度
			}
			wheel_target[0]=-sqrtf(wheel_vx * wheel_vx + wheel_vy * wheel_vy);
			wheel_target[1]=sqrtf(wheel_vx * wheel_vx + wheel_vy * wheel_vy);
			wheel_target[2]=sqrtf(wheel_vx * wheel_vx + wheel_vy * wheel_vy);
			wheel_target[3]=-sqrtf(wheel_vx * wheel_vx + wheel_vy * wheel_vy);
			//对电流进行限幅控制在-3000~3000
//        if (wheel_current > SWERVE_OPEN_LOOP_MAX_CURRENT)
//        {
//            wheel_current = SWERVE_OPEN_LOOP_MAX_CURRENT;
//        }
//        else if (wheel_current < -SWERVE_OPEN_LOOP_MAX_CURRENT)
//        {
//            wheel_current = -SWERVE_OPEN_LOOP_MAX_CURRENT;
//        }

//        if (yaw_current > SWERVE_OPEN_LOOP_MAX_CURRENT)
//        {
//            yaw_current = SWERVE_OPEN_LOOP_MAX_CURRENT;
//        }
//        else if (yaw_current < -SWERVE_OPEN_LOOP_MAX_CURRENT)
//        {
//            yaw_current = -SWERVE_OPEN_LOOP_MAX_CURRENT;
//        }
        ...
        SwerveModule[i].cmd.wheel_current_cmd = (int16_t)WheelSpeedPID[i].Out*calibration_wheel[i];
				}
				...
				//是目标值角度控制在-180~180之间
//				yaw_target_deg=-RC.ch2/660.0f*180.0f;//将遥控器数据映射成-180~180
//				    while (yaw_target_deg > 180.0f)   yaw_target_deg -= 360.0f;
//            while (yaw_target_deg < -180.0f)  yaw_target_deg += 360.0f;
				yaw_err=angle_target[i]-yaw_now;
						while (yaw_err > 180.0f)   yaw_err -= 360.0f;
            while (yaw_err < -180.0f)  yaw_err += 360.0f;
				//外环角度换环
				YawAnglePID[i].Target=yaw_err;
				YawAnglePID[i].Actual=0.0f;
				PID_Update(&YawAnglePID[i]);
				//内环速度环
				YawSpeedPID[i].Target=calibration_yaw[i]*YawAnglePID[i].Out;
...
//    {
//			//航向电机角度PID
//				 WheelSpeedPID[i].Kp=0.0f;
//				 WheelSpeedPID[i].Ki=0.0f;
//				 WheelSpeedPID[i].Kd=0.0f;
//				 PID_Init(&WheelSpeedPID[i]);
//			 //航向电机PID
//			 YawSpeedPID[i].Kp=0.0f;
//			 YawSpeedPID[i].Ki=0.0f;
//			 YawSpeedPID[i].Kd=0.0f;
//			 PID_Init(&YawSpeedPID[i]);
//			 //航向电机角度PID
//			 YawAnglePID[i].Kp=0.0f;
//			 YawAnglePID[i].Ki=0.0f;
//			 YawAnglePID[i].Kd=0.0f;
//			 PID_Init(&YawAnglePID[i]);
//    }
//		}
    ...
    SendChassisInstruction(ChassisTxBuf);//H7传数据给G0
}
